...
pl.subplot(2,4,1)
pl.imshow(img_corr,cmap='gray',vmax=np.percentile(img_corr,vmax_corr_perc),vmin=np.percentile(img_corr,vmin_corr_perc))
...
pl.axis('off')
pl.subplot(2,4,5)
pl.imshow(img_median,cmap='gray',vmax=np.percentile(img_median,vmax_median_perc),vmin=np.percentile(img_median,vmin_median_perc))
pl.imshow(np.nanmax(rois_1,-1),cmap='ocean',vmax=2,alpha=.2,vmin=0)
pl.ylabel('MEDIAN')
pl.axis('off')
pl.subplot(2,4,6)
