...
pl.close('all')
import itertools
#label_name = ['regions/natalia_active_regions.zip','regions/lindsey_active_regions.zip','regions/sonia_active_regions.zip','regions/ben_active_regions.zip']
label_name = ['regions/natalia_all_regions.zip','regions/lindsey_all_regions.zip','regions/sonia_all_regions.zip','regions/ben_all_regions.zip']
results = dict()
for count,par in enumerate(params):
    ...
    #    pl.pause(0.01)
        ...
        idx_tp_gt,idx_tp_comp, idx_fn, idx_fp_comp, performance =  cm.base.rois.nf_match_neurons_in_binary_masks(roi_nat,roi_lin,thresh_cost=.7, min_dist = 10, print_assignment= False,plot_results=True,Cn=c_img,labels = labels)
...
for count,ref_name in enumerate(['ben','lindsey','sonia','natalia']):
    ...
    vals = []
    all_values = []
    for fl in [[(r,a[0][8:-12],a[1][8:-12],b['performance']['f1_score']) for a,b in k.iteritems()] for r,k in results.iteritems()]:
...
#df = pd.DataFrame(vals, index=cols, columns=index)            
all_values = []
for fl in [[(r,a[0][8:-12],a[1][8:-12],b['performance']['f1_score'],b['performance']['precision'],b['performance']['recall']) for a,b in k.iteritems()] for r,k in results.iteritems()]:
