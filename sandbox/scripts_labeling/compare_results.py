...
fitness_ben, _, _ = cm.components_evaluation.compute_event_exceptionality(
    traces_ben)
fitness_princeton, _, _ = cm.components_evaluation.compute_event_exceptionality(
    traces_princeton)
...
idx_tp_gt, idx_tp_comp, idx_fn, idx_fp_comp, performance = cm.base.rois.nf_match_neurons_in_binary_masks(
    masks_ben, masks_cnmf, thresh_cost=.7, min_dist=10, print_assignment=False, plot_results=True, Cn=Cn)
masks_gt = masks_ben
...
pl.figure(figsize=(30, 20))
idx_tp_gt, idx_tp_comp, idx_fn, idx_fp_comp, performance = cm.base.rois.nf_match_neurons_in_binary_masks(
    masks_ben, masks_princeton, thresh_cost=.7, min_dist=10, print_assignment=False, plot_results=True, Cn=Cn)
masks_gt = masks_ben
...
pl.figure(figsize=(30, 20))
idx_tp_gt, idx_tp_comp, idx_fn, idx_fp_comp, performance = cm.base.rois.nf_match_neurons_in_binary_masks(
    masks_princeton, masks_cnmf, thresh_cost=.7, min_dist=10, print_assignment=False, plot_results=True, Cn=Cn)
...
pl.subplot(1, 2, 1)
pl.imshow(Cn)
[pl.contour(mm, levels=[0], colors='w', linewidths=2)
 for mm in masks_comp[idx_tp_comp]]
[pl.contour(mm, levels=[0], colors='r', linewidths=2)
