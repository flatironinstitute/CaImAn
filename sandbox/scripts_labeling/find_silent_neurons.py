...
rois_1 = np.transpose(cse.utilities.nf_read_roi_zip(
    base_folder + 'regions/ben_regions.zip', np.shape(Cn)), [1, 2, 0])
...
options = cse.utilities.CNMFSetParms(
    Y, n_processes, p=p, gSig=gSig, K=K, ssub=2, tsub=2)
...
C, A, b, f, S, bl, c1, neurons_sn, g, YrA = cse.temporal.update_temporal_components(
    Yr, A, b, Cin, f, dview=dview, bl=None, c1=None, sn=None, g=None, **options['temporal_params'])
...
traces = traces - \
    scipy.signal.savgol_filter(traces, np.shape(traces)[
                               1] / 2 * 2 - 1, 1, axis=1)
...
cse.utilities.view_patches_bar(Yr, scipy.sparse.coo_matrix(A.tocsc()[
...
#%%
cse.utilities.view_patches_bar(Yr, scipy.sparse.coo_matrix(A2.tocsc()[
                               :, pos_examples]), C2[pos_examples, :], b2, f2, dims[0], dims[1], YrA=YrA[pos_examples, :], img=Cn)
#%%
cse.utilities.view_patches_bar(Yr, scipy.sparse.coo_matrix(A2.tocsc()[
                               :, neg_examples]), C2[neg_examples, :], b2, f2, dims[0], dims[1], YrA=YrA[neg_examples, :], img=Cn)
