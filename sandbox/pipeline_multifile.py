...
for file_to_process in (all_files[:1] + all_files):
    ...
    pl.imshow(total_template_wls, cmap='gray',
              vmax=np.percentile(total_template_wls, 95))
    pl.pause(.01)
    count += 1
...
#%%
border_to_0 = np.max([np.ceil(np.max(np.array(ttl))).astype(np.int)
                      for ttl in total_shifts_els])
...
for ffnn in fnames_map:
    print(ffnn)
    adds_to_movie.append(np.min(cm.load(fnames_map[0])[
                         :, border_to_0:-border_to_0, border_to_0:-border_to_0]).min())
...
idx_xy = None
base_name = 'Yr'
name_new = cm.save_memmap_each(fnames_map, dview=dview_sub, base_name=base_name, resize_fact=(
...
if len(name_new) > 1:
    fname_new = cm.save_memmap_join(
        name_new, base_name='Yr', n_chunks=56, dview=dview)
else:
...
#%%
t1 = time.time()
all_files_mmap_single = [os.path.abspath(
    flfl) for flfl in glob.glob('Yr0*.mmap')]
...
#%%
np.savez(os.path.join(os.path.split(fname_new)[0], os.path.split(fname_new)[1][:-4] + 'results_analysis_patch.npz'), Cn=Cn, A_tot=A_tot.todense(
), C_tot=C_tot, b_tot=b_tot, f_tot=f_tot, YrA_tot=YrA_tot, sn_tot=sn_tot, d1=d1, d2=d2, idx_components=idx_components, idx_components_bad=idx_components_bad)
#%%
if False:
...
crd = plot_contours(A.tocsc()[:, idx_components],
                    Cn, thr=0.9, cmap='gray', colors='r')
crd = plot_contours(scipy.sparse.coo_matrix(roi_lindsey.transpose([1, 2, 0]).reshape(
    (d1 * d2, -1), order='F')), Cn, thr=0.99999, cmap='gray', colors='y')
#%%
for count, nrn in enumerate([998, 989, 987, 976, 963, 962]):
    pl.subplot(6, 2, 2 * count + 1)
    pl.imshow(A.tocsc()[:, idx_components[nrn]
                        ].toarray().reshape((d1, d2), order='F'))
    pl.subplot(6, 2, 2 * count + 2)
    pl.plot(scipy.signal.resample_poly(
        traces[idx_components[nrn]], 1, 1)[::10])
