...
mc = cm.load('/mnt/ceph/users/agiovann/ImagingData/DanGoodwin/Somagcamp-Fish4-z13-100-400crop256.tif')[
    :2000].motion_correct(3, 3)[0]
#%%
import cv2
import numpy as np
...
inputImage = m[10]
mapX = np.zeros(
    (num_frames, inputImage.shape[0], inputImage.shape[1]), dtype=np.float32)
mapY = np.zeros(
    (num_frames, inputImage.shape[0], inputImage.shape[1]), dtype=np.float32)
templ = np.median(m, 0)
map_orig_x = np.zeros(
