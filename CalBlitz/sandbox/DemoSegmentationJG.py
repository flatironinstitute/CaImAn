...
initTime=time.time()
m,mbl=m.computeDFF(secsWindow=40,quantilMin=8,method='delta_f_over_sqrt_f')
...
#%% visualize PCs
spcomps=m.IPCA(components=180,batch=1e+7)
T,h,w=m.shape;
...
initTime=time.time()
spcomps=m.IPCA_stICA(componentsPCA=180,componentsICA = 150, mu=.5, batch=1000000, algorithm='parallel', whiten=True, ICAfun='logcosh', fun_args=None, max_iter=1000, tol=1e-8, w_init=None, random_state=None);
print 'elapsed time:' + str(time.time()-initTime) 
...
_masks,_=cb.extractROIsFromPCAICA(spcomps, numSTD=4.0, gaussiansigmax=.2 , gaussiansigmay=.2)
...
#%% 
pl.imshow(np.sum(all_masksForPlot_tmp,axis=0))
#%%
STOP HERE
