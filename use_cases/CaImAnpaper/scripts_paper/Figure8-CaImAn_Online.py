...
base_folder = '/mnt/ceph/neuro/DataForPublications/DATA_PAPER_ELIFE/timings_online'
...
ds_ind = [0,2,6,7,8]   #  indeced of datasets that were spatially downsampled
nd_ind = [1,3,4,5]
area = 40*np.ones(len(N))
area[nd_ind] *= 2

_, ax = plt.subplots()
ax.scatter(np.array(N)[ds_ind], np.array(fps)[ds_ind], s=np.array(area)[ds_ind], label='with downsampling')
ax.scatter(np.array(N)[nd_ind], np.array(fps)[nd_ind], s=np.array(area)[nd_ind], label='w/o downsampling')
ax.legend()
