...
#        m_res_bg = m_orig - cm.movie(np.reshape(A_gt.dot(
#            C_gt) + b_gt.dot(f_gt), dims + (-1,), order='F').transpose([2, 0, 1]))
        #%
...
#            return ain.reshape(dims,order = 'F'), cin, cin_res
        ...
        peaks_idx = [peakutils.indexes(-exceptional, thres = 50/np.max(-exceptional)) for exceptional in exceptionality]
        peaks_idx_exc = [-exceptional[pk] for exceptional,pk in zip(exceptionality, peaks_idx)]
        peaks_new = []
        for pks,pks_exc in zip(peaks_idx, peaks_idx_exc):
            new_pks = pks[pks_exc>100]
            new_pks = np.union1d(new_pks,pks[np.argsort(pks_exc)][-3:])
            peaks_new.append(new_pks)

        peaks_idx = [pk if len(pk)>0 else [np.argmax(-exc_orig)] for pk,exc_orig in zip(peaks_new, exceptionality)]
        peaks_idx = [np.add(pk,count_start//2) for pk in peaks_idx]

#        peaks_idx = [[np.argmax(-exceptional)+count_start//2] for exceptional in exceptionality]
...
#            img_temp = img_avg.copy().astype(np.float32)/max_mov*10


            possibly_active = np.where((exceptionality[:, count - count_start:count].min(-1) < -10) & (
                exceptionality[:, count - count_start:count].min(-1) >= -50))[0]

            if use_peaks:
                super_active = np.where([count in pk for pk in  peaks_idx])
            else:
...
#            cv2.imshow('frame', cv2.resize(img_temp*1,(dims[1]*2,dims[0]*2)))
#            cv2.waitKey(time_wait)
#            time_wait = 1
        np.savez(base_name+'_strict_peak.npz',all_dubious_crops = all_dubious_crops, all_pos_crops = all_pos_crops, all_neg_crops = all_neg_crops, gSig = gSig)
...
#            all_dubious_crops = ld['all_dubious_crops']
            ...
            for class_id, pos in enumerate([all_pos_crops, all_neg_crops]):
                ...
                total_crops += [cv2.resize(ain,(patch_size, patch_size)) for ain in pos]
...
np.savez('use_cases/CaImAnpaper/groun_truth_sniper/residual_crops_sniper_selective.npz',
