...
t2 = time.time() - t1
print(t2)
pl.imshow(total_template_rig, cmap='gray', vmin=np.percentile(
...
for num_splits_to_process in num_splits_to_process_list[-1:]:
    ...
    pl.imshow(new_templ, cmap='gray', vmin=np.percentile(
        new_templ, 5), vmax=np.percentile(new_templ, 95))
    pl.pause(.1)
t2 = time.time() - t1
print(t2)
#%%
...
m_els.resize(1, 1, downs).play(
    fr=20, gain=5, magnification=1, offset=add_to_movie)
#%%
cm.movie(np.array(templates_els)).play(
    fr=2, gain=5, magnification=1, offset=add_to_movie)

#%%
m_els = cm.load(fname_tot_els)
downs = .2
cm.concatenate([m_orig.resize(1, 1, downs), m_rig.resize(1, 1, downs), m_els.resize(
    1, 1, downs)], axis=2).play(fr=10, gain=1, magnification=2, offset=add_to_movie)
