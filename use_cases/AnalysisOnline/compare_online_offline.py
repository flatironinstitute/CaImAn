...
with np.load(online_file) as ld:
    ...
    if not np.all(dims_on == dims_off):
        Ab = scipy.sparse.csc_matrix(np.concatenate([cv2.resize(Ab[:, comp_id].toarray().reshape(
            dims_on, order='F'), dims_off[::-1]).reshape((-1, 1), order='F') for comp_id in range(Ab.shape[-1])], axis=1))
...
C_on_thr = np.array([CC.reshape([-1, n_frames_per_bin]).max(1)
                     for CC in C_on_thr])
#%%
pl.figure()
...
roi_rs = nf_read_roi_zip(
    '/mnt/ceph/neuro/labeling/k53_20160530/regions/intermediate_regions/sonia_active_regions.zip', Cn.shape)
print(roi_rs.shape)
roi_bs = nf_read_roi_zip(
    '/mnt/ceph/neuro/labeling/k53_20160530/regions/intermediate_regions/lindsey_active_regions.zip', Cn.shape)
print(roi_bs.shape)
roi_ds = nf_read_roi_zip(
    '/mnt/ceph/neuro/labeling/k53_20160530/regions/intermediate_regions/natalia_active_regions.zip', Cn.shape)
print(roi_bs.shape)
...
#%%
roi_rs = nf_read_roi_zip(
    '/mnt/ceph/neuro/labeling/J115_2015-12-09_L01_ELS/regions/intermediate_regions/sonia_active_regions.zip', Cn.shape)
print(roi_rs.shape)
roi_bs = nf_read_roi_zip(
    '/mnt/ceph/neuro/labeling/J115_2015-12-09_L01_ELS/regions/intermediate_regions/lindsey_active_regions.zip', Cn.shape)
print(roi_bs.shape)
roi_ds = nf_read_roi_zip(
    '/mnt/ceph/neuro/labeling/J115_2015-12-09_L01_ELS/regions/intermediate_regions/natalia_active_regions.zip', Cn.shape)
print(roi_ds.shape)
#%%
plot_results = False
#%%
...
#%%
pl.figure(figsize=(30, 20))
idx_tp_gt, idx_tp_comp, idx_fn, idx_fp_comp, performance_off_on = cm.base.rois.nf_match_neurons_in_binary_masks(A_off_thr[:, :].reshape([dims_off[0], dims_off[1], -1], order='F').transpose(
    [2, 0, 1]) * 1., A_on_thr[:, :].reshape([dims_on[0], dims_on[1], -1], order='F').transpose([2, 0, 1]) * 1., thresh_cost=.7, min_dist=10, print_assignment=False, plot_results=True, Cn=Cn, labels=['online', 'offline'])
pl.rcParams['pdf.fonttype'] = 42
font = {'family': 'Myriad Pro',
        'weight': 'regular',
        'size': 20}
...
#%%
corrs = np.array([scipy.stats.pearsonr(C_off_thr[gt, :], C_on_thr[comp, :])[
                 0] for gt, comp in zip(idx_tp_gt, idx_tp_comp)])
#%%
