...
traces_mat, time_mat = gc.extract_traces_mat(
    traces_DFF, triggers_img[:, -1], f_rate, time_before=time_before, time_after=time_after)
traces_mat, time_mat = scipy.signal.resample(
    traces_mat, len(time_w_mat), t=time_mat, axis=-1)
...
# [169, 249, 392, 600, 434,  17, 907, 755, 834,  58, 586, 404, 737]:
for idx in [126]:
...
ax = pl.gca()
ax.plot(np.tile([0, 1], (mean_up.shape[0], 1)).T, old_div(
    np.vstack([mean_down, mean_up]), .2), '-ro', markersize=20)
