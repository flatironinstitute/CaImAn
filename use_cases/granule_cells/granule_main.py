...
for base_folder in base_folders:
    ...
    #%%
    triggers_img, trigger_names_img = gc.extract_triggers(
        fls, read_dictionaries=False)
    np.savez(base_folder + 'all_triggers.npz',
             triggers=triggers_img, trigger_names=trigger_names_img)
    ...
    res_bt = gc.get_behavior_traces(
        camera_file[0], t0=0, t1=8.0, freq=60, ISI=.25, draw_rois=False, plot_traces=False, mov_filt_1d=True, window_lp=5)
    t_end = time() - t_start
    print(t_end)
    #%%
    ...
    B_s, lab_imgs, cm_s = gc.threshold_components(
        A_s, shape, min_size=5, max_size=50, max_perc=.5)
    #%%
    if not batch_mode:
        for i, A_ in enumerate(B_s):
            sizes = np.array(A_.sum(0)).squeeze()
            pl.subplot(2, 3, i + 1)
            pl.imshow(np.reshape(A_.sum(1), shape, order='F'),
                      cmap='gray', vmax=.5)
    ...
    if len(B_s) > 1:
        ...
        #%%
        neurons = gc.link_neurons(
            matches, costs, max_cost=0.6, min_FOV_present=None)
    else:
        neurons = [np.arange(B_s[0].shape[-1])]
    #%%
    np.savez(base_folder + 'neurons_matching.npz', matches=matches,
             costs=costs, neurons=neurons, D_s=D_s)
    #%%
    re_load = False
    if re_load:
        import calblitz as cb
        ...
        B_s, lab_imgs, cm_s = gc. threshold_components(
            A_s, shape, min_size=5, max_size=50, max_perc=.5, remove_unconnected_components=remove_unconnected_components)
    #%%
        ...
        for neuro in range(num_neurons):
            for idx, B in enumerate(A_s):
                ...
                pl.imshow(np.reshape(
                    B[:, neurons[idx][neuro]].sum(1), shape, order='F'))
            pl.pause(.01)
            for idx, B in enumerate(A_s):
                pl.subplot(row_cols, row_cols, idx + 1)
                pl.cla()
    ...
    if 0:
        ...
        for row, column in zip(matches[idx][0], matches[idx][1]):
            ...
            if value < .5:
                pl.cla()
                pl.imshow(np.reshape(B_s[idx][:, row].todense(
                ), (512, 512), order='F'), cmap='gray', interpolation='None')
                pl.imshow(np.reshape(B_s[idx + 1][:, column].todense(), (512, 512),
                                     order='F'), alpha=.5, cmap='hot', interpolation='None')
    ...
    #    YrA=YrA_s[idx][neurons[idx]]
        ...
        traces_, traces_DFF_, traces_BL_ = gc.generate_linked_traces(
    ...
    if interpolate:
        ...
        Ftraces_mat = np.zeros([len(chunk_sizes), len(
            traces[0]), samples_after + samples_before])
        ...
        if interpolate:
            ...
            if fr != max_chunk:
                ...
                f1 = scipy.interpolate.interp1d(
                    np.arange(fr), Ftraces[idx], axis=1, bounds_error=False, kind='linear')
    ...
    wheel_traces, movement_at_CS, trigs_mov = gc.process_wheel_traces(np.array(
        res_bt['wheel']), tm, thresh_MOV_iqr=1000, time_CS_on=-.25, time_US_on=0)
    print(trigs_mov)
    ...
    ftraces = ftraces - \
        np.median(ftraces[:, :, :samples_before - ISI],
                  axis=(2))[:, :, np.newaxis]
    amplitudes_responses = np.mean(
        ftraces[:, :, samples_before + ISI - 1:samples_before + ISI + 1], -1)
    ...
    amplitudes_responses = np.mean(
        ftraces[:, :, samples_before + ISI - 1:samples_before + ISI + 1], -1)
    #%%
    np.savez('ftraces.npz', ftraces=ftraces,
             samples_before=samples_before, samples_after=samples_after, ISI=ISI)
    ...
    if not batch_mode:
        ...
        for cell in range(ftraces.shape[1]):
            ...
            pl.imshow(ftraces[np.concatenate([uss, nocrs, crs]),
                              cell, :], aspect='auto', vmin=0, vmax=1)
    ...
    if not batch_mode:
        ...
        for resps in amplitudes_responses.T:
            r = np.corrcoef(
                amplitudes_at_US[idxCSCSUS], resps[idxCSCSUS])[0, 1]
        #    if r>.25:
