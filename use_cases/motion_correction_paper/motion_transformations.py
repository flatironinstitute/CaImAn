...
def apply_field(inputImage,vx,vy):
    ...
    mapX =  map_orig_x + vx
    mapY =  map_orig_y + vy
    new_img = cv2.remap(inputImage, mapX, mapY, cv2.INTER_CUBIC, None, cv2.BORDER_CONSTANT)
    return new_img
#%%
...
if False:
    #%%
    import caiman as cm
    m = cm.load('/mnt/ceph/neuro/Sue_2000_els_opencv__d1_512_d2_512_d3_1_order_F_frames_2000_.hdf5').resize(1,1,.1)
...
#    min_val = np.min(new_templ) 
    ...
    for count in [max_shift-1]:
        print(count)
        vx_,vy_ =  0,0
        vx1,vy1 = create_rotation_field(max_displacement=(count%max_shift,count%max_shift),center=(0,0),nx=512,ny=512)
        vx_ += vx1
        vy_ += vy1    
...
#        vy_ /= 2
#
        pl.subplot(1,2,1);pl.imshow(vx_,interpolation='none');pl.colorbar();pl.subplot(1,2,2);pl.imshow(vy_,interpolation='none');pl.colorbar()   
#        break
...
for patch_size in np.array([24,48,96,128]):
    ...
    best_possible = np.dstack([best_possible_x*array  ,best_possible_y*array ])
    ...
    for patch_size in [32,48,96,128]:          
            for noise_sigma in np.arange(0.01,4,.4,dtype = np.float32):
                for iter_ in range(10):
                        ...
                        rand_add = np.random.randn(*new_templ.shape).astype(np.float32)*noise_sigma*(new_templ)
                        ...
                        mov_dist['gt_shifts_masked'] = np.dstack([vx*array  ,vy*array ])                        
                        mov_dist['img_flow'] = img_flow 
                        print(mov_dist['patch_size'],mov_dist['noise_sigma'],mov_dist['iter_'])
                        all_movs_dist.append(mov_dist)
    #%%
    import scipy
    scipy.io.savemat('/mnt/ceph/neuro/DataForPublications/Piecewise-Rigid-Analysis-paper/Simulations/test_simulation_shifts_nrmcorre.mat',{'all_movs_tosave':all_movs_tosave,'template':new_templ,'mask':mask})
    np.savez('/mnt/ceph/neuro/DataForPublications/Piecewise-Rigid-Analysis-paper/Simulations/test_simulation_shifts_nrmcorre.npz',{'all_movs_dist':all_movs_dist,'template':new_templ,'mask':mask})
    ...
    for mov_dist in all_movs_dist:
        ...
        shifts_oflow = caiman.motion_correction.compute_flow_single_frame(mov_dist['newim']*255,mov_dist['template']*255)
        shifts_oflow_masked = np.dstack([shifts_oflow[:,:,0]*array  ,shifts_oflow[:,:,1]*array ])    
        mov_dist['shifts_oflow'] = shifts_oflow
        mov_dist['shifts_oflow_masked'] = shifts_oflow_masked
        mov_dist['norm_diff_shifts_oflow'] = np.linalg.norm(shifts_oflow_masked+mov_dist['gt_shifts_masked'])/np.linalg.norm(mov_dist['gt_shifts_masked'])
        print(mov_dist['norm_diff_shifts_oflow'])
        cv2.imshow('frame',np.concatenate([img_flow,dispOpticalFlow(np.dstack([mov_dist['newim'],mov_dist['newim'],mov_dist['newim']]),5*(shifts_oflow_masked+mov_dist['gt_shifts_masked']),Divisor=20)],axis=0))
        cv2.waitKey(1)
...
#        shifts_ncorre_masked= np.dstack([cv2.resize(sh_efty[0],new_templ.shape)*array  ,cv2.resize(sh_efty[1],new_templ.shape)*array ]).T
        ...
        cv2.imshow('frame',np.concatenate([img_flow,dispOpticalFlow(np.dstack([mov_dist['newim'],mov_dist['newim'],mov_dist['newim']]),10*(shifts_ncorre_masked-mov_dist['gt_shifts_masked']),Divisor=20)],axis=0))
        cv2.waitKey(1)
        
        correct_img[np.isnan(correct_img)] = np.nanmean(correct_img)
        shifts_oflow_res = caiman.motion_correction.compute_flow_single_frame(correct_img*255,mov_dist['template']*255)
        shifts_oflow_res_masked = np.dstack([shifts_oflow_res[:,:,0]*array  ,shifts_oflow_res[:,:,1]*array ])    
...
#        print(mov_dist['norm_diff_shifts_ncorre'] )      
#%%
        np.savez('/mnt/ceph/neuro/DataForPublications/Piecewise-Rigid-Analysis-paper/Simulations/res_rotation_total.npz', all_movs_dist = all_movs_dist, max_shift = max_shift)        
#%%
...
#    newim[:,50:100] = (np.concatenate([newim[3:,50:100],newim[:3,50:100]],axis = 0))
    ...
    newim[::2,:] = (np.concatenate([newim[::2,-3:],newim[::2,:-3]],axis = 1))    
...
#    sequences = [sima.Sequence.create('ndarray',np.repeat(np.concatenate([new_templ[None,None,:,:,None],newim[None,None,:,:,None]],axis = 0),10,axis = 0))]
    ...
    mc_approach  = sima.motion.HiddenMarkov2D(granularity = (granularity,gran_n),max_displacement=[max_shift]*2, verbose = True,n_processes  = 14)
