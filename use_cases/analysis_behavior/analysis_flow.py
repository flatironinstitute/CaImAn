...
try:
    ...
    print('Not running under iPython')
...
for e in gt_names:
    ...
    spatial_filter_, time_trace_, of_or = cm.behavior.behavior.extract_motor_components_OF(
        m, n_components, mask=mask,  resize_fact=resize_fact, only_magnitude=only_magnitude, max_iter=1000, verbose=True)
...
corr_whisk_large_fov= [0.9	, 0.91035296	, 0.9439634281	, 0.7447611, 0.49	, 0.81198401	, 0.8385994053	, 0.61250132, 0.31	, 0.53200557	, 0.5008999795	, 0.29134304, 0.49	, 0.63037966	, 0.5307278583	, 0.84339566, 0.69	, 0.6405778	, 0.7191003501	, 0.58845782, 0.67	, 0.87805005	, 0.9112280212, 	0.67212455]
...
#        pl.plot(wheel_pix_sess,'c')
    ...
    if not mask_all:
        np.savez(os.path.join(e, 'results_wheel_2.npy'), mag=mag, dirct=dirct, wheel_pix_sess=wheel_pix_sess,
                 spatial_filter=spatial_filter, time_trace=time_trace, mask=mask, cc=cc, of=of, of_or=of_or)
    else:
        np.savez(os.path.join(e, 'results_wheel_all.npy'), mag=mag, dirct=dirct, wheel_pix_sess=wheel_pix_sess,
                 spatial_filter=spatial_filter, time_trace=time_trace, mask=mask, cc=cc)
