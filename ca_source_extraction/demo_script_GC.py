...
#%%
m=cb.load('M_FLUO.tif',fr=15,subindices=range(3000,5000,1)); 
T,h,w=np.shape(m)
m1=m[:,:h/2,:]
m2=m[:,h/2:,:]
m=m2;
...
nr = 70
t1 = time()
Ain,Cin,center = greedyROI2d(Y, nr = nr, gSig = [2,2], gSiz = [3,3], use_median = False)
t_elGREEDY = time()-t1
...
#%%
t1 = time()
A,b = update_spatial_components(Yr, Cin, fin, Ain, d1=d1, d2=d2, sn = P['sn'],dist=2,max_size=5,min_size=3)
t_elSPATIAL = time() - t1
#%%
...
t1 = time()
A_sp=A.tocsc();
A_m,C_m,nr_m,merged_ROIs,P_m=mergeROIS(Y_res,A_sp,b,np.array(C),f,d1,d2,Pnew,sn=P['sn'],thr=.95,deconv_method='spgl1')
t_elMERGE = time() - t1
#%%
crd = plot_contours(A_m,Cn,thr=0.9)
#%%
A2,b2 = update_spatial_components(Yr, C_m, f, A_m, d1=d1, d2=d2, sn = P['sn'],dist=2,max_size=2)
