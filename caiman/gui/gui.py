...
class GUI(animation.TimedAnimation):
    ...
    def __init__(self, mov, roi, traces, images={}, cmap=pl.cm.viridis, **kwargs):
        ...
        gs.update(wspace=0.1, hspace=0.1, left=.04,
                  right=.96, top=.98, bottom=.02)
        ...
        self.axs_imbuts = [self.fig.add_subplot(
            gs[110:128, idx * 2:idx * 2 + 2]) for idx, i in enumerate(self.images)]
        ...
        self.sl_contrast0 = Slider(
            self.ax_contrast0, 'Low', 0., 255.0, valinit=self.c0, valfmt='%d')
        self.sl_contrast1 = Slider(
            self.ax_contrast1, 'Hi', 0., 255.0, valinit=self.c1, valfmt='%d')
        self.sl_contrast0.on_changed(self.evt_contrast)
        self.sl_contrast1.on_changed(self.evt_contrast)
        self.img_buttons = [Button(ax, k) for k, ax in zip(
            list(self.images.keys()), self.axs_imbuts)]
        ...
        self.roidata = self.ax_mov.imshow(
            self.roi, cmap=self.cmap, alpha=0.5, vmin=np.nanmin(self.roi), vmax=np.nanmax(self.roi))
        self.trdata, = self.ax_trc.plot(np.zeros(len(self.traces)))
        self.navdata, = self.ax_nav.plot(
            [-2, -2], [-1, np.max(self.traces)], 'r-')
        if len(self.images):
        ...
        animation.TimedAnimation.__init__(
            self, self.fig, interval=40, blit=True, **kwargs)
    ...
    def set_current_trace(self, idx):
        col = self.cmap(np.linspace(0, 1, np.sum(self.roi_kept)))[
            np.squeeze(idx)]
        t = self.traces[:, idx]
    ...
    def plot_current_traces(self):
        ...
        cols = self.cmap(np.linspace(0, 1, len(self.roi_idxs)))[
            self.current_roi]
        lastmax = 0
