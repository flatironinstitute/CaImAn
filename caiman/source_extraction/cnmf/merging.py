...
#                nA = np.sqrt(np.sum(np.array(aa)**2)
                ...
                if g is not None:
                    cc,bm,cm,gm,sm,ss,_ = constrained_foopsi(np.array(cc).squeeze(),g=g[merged_ROI[indx]],**temporal_params)
                else:
                    cc,bm,cm,gm,sm,ss,_ = constrained_foopsi(np.array(cc).squeeze(),g=None,**temporal_params)
                ...
                cc,_,_,__,_,bl__,c1__,sn__,g__,YrA,_ = update_temporal_components(np.asarray(Y_res[ff,:]),A_merged[ff,i],b[ff],aa_2,f,dview=dview,bl=None,c1=None,sn=None,g=None,**temporal_params)
                aa,bb,cc,f = update_spatial_components(np.asarray(Y_res),cc,f,A_merged[:,i],sn=sn_pix,dview=dview,**spatial_params)
                A_merged[:,i] = aa.tocsr();                
                cc,_,_,_,_,ss,bl__,c1__,sn__,g__,YrA,_ = update_temporal_components(Y_res[ff,:],A_merged[ff,i],bb[ff],cc,f,dview=dview,bl=bl__,c1=c1__,sn=sn__,g=g__,**temporal_params)
