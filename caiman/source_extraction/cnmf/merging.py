...
#        P_merged=[];
        ...
        for i in range(nm):
            ...
            nC = np.sqrt([cc.dot(cc) for cc in C[merged_ROI]])
    #        A_merged[:,i] = np.squeeze((A[:,merged_ROI]*spdiags(nC,0,len(nC),len(nC))).sum(axis=1))
            if fast_merge:
                ...
                for iter in range(10):
                    cc = Acsc.T.dot(aa).T.dot(Ctmp) / (aa.T * aa)
                    aa = Acsc.dot(Ctmp.dot(cc.T)) / (cc * cc.T)

                nC = np.sqrt(np.ravel(Acsc.power(2).sum(axis=0)) * np.sum(Ctmp**2, axis=1))
                # nA = np.sqrt(old_div(np.sum(np.array(aa)**2), A.tocsc()
                #                      [:, merged_ROIs[i]].power(2).sum(0).max()))
                nA = np.sqrt(aa.T.dot(aa)[0, 0] / Acsc.power(2).sum(0).max())
                aa /= nA
                cc *= nA
                ...
                if g is not None:
                    cc, bm, cm, gm, sm, ss = constrained_foopsi(
                        np.array(cc).squeeze(), g=g[merged_ROI[indx]], **temporal_params)
                else:
                    cc, bm, cm, gm, sm, ss = constrained_foopsi(
                ...
                aa_1 = scipy.sparse.linalg.spsolve(scipy.sparse.diags(
                    nC, 0, (len(nC), len(nC))), csc_matrix(C[merged_ROI, :]))
                aa_2 = (aa_1).mean(axis=0)
                ff = np.nonzero(A_merged[:, i])[0]
                cc, _, _, __, _, bl__, c1__, sn__, g__, YrA = update_temporal_components(np.asarray(Y_res[ff, :]), A_merged[
                                                                                         ff, i], b[ff], aa_2, f, dview=dview, bl=None, c1=None, sn=None, g=None, **temporal_params)
                aa, bb, cc, f = update_spatial_components(np.asarray(Y_res), cc, f, A_merged[
                                                          :, i], sn=sn_pix, dview=dview, **spatial_params)
                A_merged[:, i] = aa.tocsr()
                cc, _, _, _, _, ss, bl__, c1__, sn__, g__, YrA = update_temporal_components(Y_res[ff, :], A_merged[
                                                                                            ff, i], bb[ff], cc, f, dview=dview, bl=bl__, c1=c1__, sn=sn__, g=g__, **temporal_params)
