...
#                                 Loading Object From File")
    ...
    def _prepare_object(self, Yr, T, new_dims=None, idx_components=None):
        ...
        if expected_comps <= self.N + self.params.get('online',
                                                      'max_num_added'):
            expected_comps = self.N + self.params.get('online',
                                                      'max_num_added') + 200
        ...
        if new_dims is not None:
            ...
            for ffrr in range(T):
                tmp = cv2.resize(Yr[:, ffrr].reshape(self.dims, order='F'),
                                 new_dims[::-1])
                print(tmp.shape)
            ...
            self.estimates.b = cv2.resize(self.estimates.b,
                                          new_dims[::-1]).reshape([-1, 1],
                                                                  order='F')
        ...
        self.estimates.noisyC = np.zeros(
            (self.params.get('init', 'nb') + expected_comps, T),
            dtype=np.float32)
        self.estimates.C_on = np.zeros((expected_comps, T), dtype=np.float32)

        self.estimates.noisyC[self.params.get('init', 'nb'):self.M, :self.params
                              .get('online', 'init_batch'
                                  )] = self.estimates.C + self.estimates.YrA
        self.estimates.noisyC[:self.params.get('init', 'nb'), :self.params.
                              get('online', 'init_batch')] = self.estimates.f
        ...
        if self.params.get('preprocess', 'p'):
            ...
            self.estimates.OASISinstances = [
                OASIS(
                    g=np.ravel(0.01)
                    if self.params.get('preprocess', 'p') == 0 else gam[0],
                    lam=0 if not use_L1 else l,
                    s_min=0 if use_L1 else
                    (self.params.get('temporal', 's_min')
                     if self.params.get('temporal', 's_min') > 0 else
                     (-self.params.get('temporal', 's_min') * sn *
                      np.sqrt(1 - np.sum(gam)))),
                    b=b,
                    g2=0 if self.params.get('preprocess', 'p') < 2 else gam[1])
                for gam, l, b, sn in
                zip(self.estimates.g, self.estimates.lam, self.estimates.bl,
                    self.estimates.neurons_sn)
            ]

            for i, o in enumerate(self.estimates.OASISinstances):
                o.fit(
                    self.estimates.noisyC[i +
                                          self.params.get('init', 'nb'), :self.
                                          params.get('online', 'init_batch')])
                self.estimates.C_on[i, :self.params.get('online', 'init_batch'
                                                       )] = o.c
        else:
            self.estimates.C_on[:self.N, :self.params.
                                get('online', 'init_batch')] = self.estimates.C
        ...
        self.estimates.Ab, self.ind_A, self.estimates.CY, self.estimates.CC = init_shapes_and_sufficient_stats(
            Yr[:, :self.params.get('online', 'init_batch')].reshape(
                self.params.get('data', 'dims') + (-1,),
                order='F'), self.estimates.A,
            self.estimates.C_on[:self.N, :self.params.
                                get('online', 'init_batch')], self.estimates.b,
            self.estimates.noisyC[:self.params.get('init', 'nb'), :self.params.
                                  get('online', 'init_batch')])

        self.estimates.CY, self.estimates.CC = self.estimates.CY * 1. / self.params.get(
            'online', 'init_batch'), 1 * self.estimates.CC / self.params.get(
                'online', 'init_batch')

        print('Expecting ' + str(expected_comps) + ' components')
        self.estimates.CY.resize([
            expected_comps + self.params.get('init', 'nb'),
            self.estimates.CY.shape[-1]
        ])
        if self.params.get('online', 'use_dense'):
            self.estimates.Ab_dense = np.zeros(
                (self.estimates.CY.shape[-1],
                 expected_comps + self.params.get('init', 'nb')),
                dtype=np.float32)
            self.estimates.Ab_dense[:, :self.estimates.Ab.
                                    shape[1]] = self.estimates.Ab.toarray()
        self.estimates.C_on = np.vstack([
            self.estimates.noisyC[:self.params.get('init', 'nb'), :],
            self.estimates.C_on.astype(np.float32)
        ])
        ...
        self.params.set(
            'init', {
                ...
                np.add(
                    np.multiply(
                        np.ceil(self.params.get('init', 'gSig')).astype(np.int),
                        2), 1)
            })

        self.estimates.Yr_buf = RingBuffer(
            Yr[:,
               self.params.get('online', 'init_batch') -
               self.params.get('online', 'minibatch_shape'):self.params.
               get('online', 'init_batch')].T.copy(),
            self.params.get('online', 'minibatch_shape'))
        self.estimates.Yres_buf = RingBuffer(
            self.estimates.Yr_buf - self.estimates.Ab.dot(
                self.estimates.
                C_on[:self.M,
                     self.params.get('online', 'init_batch') -
                     self.params.get('online', 'minibatch_shape'):self.params.
                     get('online', 'init_batch')]).T,
            self.params.get('online', 'minibatch_shape'))
        ...
        self.estimates.rho_buf = imblur(
            np.maximum(self.estimates.Yres_buf.T,
                       0).reshape(self.params.get('data', 'dims') + (-1,),
                                  order='F'),
            sig=self.params.get('init', 'gSig'),
            siz=self.params.get('init', 'gSiz'),
            nDimBlur=len(self.params.get('data', 'dims')))**2
        self.estimates.rho_buf = np.reshape(
            self.estimates.rho_buf,
            (np.prod(self.params.get('data', 'dims')), -1)).T
        self.estimates.rho_buf = np.ascontiguousarray(self.estimates.rho_buf)
        self.estimates.rho_buf = RingBuffer(
            self.estimates.rho_buf, self.params.get('online',
                                                    'minibatch_shape'))
        self.estimates.AtA = (self.estimates.Ab.T.dot(
            self.estimates.Ab)).toarray()
        self.estimates.AtY_buf = self.estimates.Ab.T.dot(
            self.estimates.Yr_buf.T)
        self.estimates.sv = np.sum(
            self.estimates.rho_buf.get_last_frames(
                min(self.params.get('online', 'init_batch'),
                    self.params.get('online', 'minibatch_shape')) - 1), 0)
        self.estimates.groups = list(
            map(list,
                update_order(self.estimates.Ab)[0]))
        self.update_counter = 2**np.linspace(0, 1, self.N, dtype=np.float32)
        self.time_neuron_added: List = []
        for nneeuu in range(self.N):
            self.time_neuron_added.append(
                (nneeuu, self.params.get('online', 'init_batch')))
        if self.params.get('online', 'dist_shape_update'):
            self.time_spend = 0
            self.comp_upd: List = []
        # setup per patch classifier

        if self.params.get('online',
                           'path_to_model') is None or self.params.get(
                               'online', 'sniper_mode') is False:
            loaded_model = None
            ...
            if use_keras:
                path = self.params.get('online',
                                       'path_to_model').split(".")[:-1]
                ...
                self.tf_in = loaded_model.get_tensor_by_name(
                    'prefix/conv2d_1_input:0')
                self.tf_out = loaded_model.get_tensor_by_name(
                    'prefix/output_node0:0')
    ...
    def fit_next(self, t, frame_in, num_iters_hals=3):
        ...
        if (not self.params.get('online', 'simultaneously')) or self.params.get(
                'preprocess', 'p') == 0:
            # get noisy fluor value via NNLS (project data on shapes & demix)
            C_in = self.estimates.noisyC[:self.M, t - 1].copy()
            self.estimates.C_on[:self.M,
                                t], self.estimates.noisyC[:self.
                                                          M, t] = HALS4activity(
                                                              frame,
                                                              self.estimates.Ab,
                                                              C_in,
            ...
            (self.estimates.C_on[:self.M, t + 1 - mbs:t + 1],
             ...
             self.estimates.OASISinstances) = demix_and_deconvolve(
                 self.estimates.C_on[:self.M, t + 1 - mbs:t + 1],
                 self.estimates.noisyC[:self.M, t + 1 - mbs:t + 1],
                 self.estimates.AtY_buf,
                 self.estimates.AtA,
                 self.estimates.OASISinstances,
                 iters=num_iters_hals,
        ...
        #self.estimates.mean_buff = self.estimates.Yres_buf.mean(0)
        res_frame = frame - self.estimates.Ab.dot(
            self.estimates.C_on[:self.M, t])
        mn_ = self.estimates.mn.copy()
        self.estimates.mn = (t - 1) / t * self.estimates.mn + res_frame / t
        self.estimates.vr = (t - 1) / t * self.estimates.vr + (
            res_frame - mn_) * (res_frame - self.estimates.mn) / t
        self.estimates.sn = np.sqrt(self.estimates.vr)

        t_new = time()
        if self.params.get('online', 'update_num_comps'):

            self.estimates.mean_buff += (
                res_frame - self.estimates.Yres_buf[self.estimates.Yres_buf.cur]
            ) / self.params.get('online', 'minibatch_shape')
            self.estimates.Yres_buf.append(res_frame)

            res_frame = np.reshape(res_frame,
                                   self.params.get('data', 'dims'),
                                   order='F')

            rho = imblur(np.maximum(res_frame, 0),
                         sig=self.params.get('init', 'gSig'),
                         siz=self.params.get('init', 'gSiz'),
                         nDimBlur=len(self.params.get('data', 'dims')))**2
            ...
            (self.estimates.Ab, Cf_temp, self.estimates.Yres_buf,
             self.estimates.rho_buf, self.estimates.CC, self.estimates.CY,
             self.ind_A, self.estimates.sv, self.estimates.groups,
             self.estimates.ind_new, self.ind_new_all, self.estimates.sv,
             self.cnn_pos) = update_num_components(
                 t,
                 self.estimates.sv,
                 self.estimates.Ab,
                 self.estimates.C_on[:self.M, (t - mbs + 1):(t + 1)],
                 self.estimates.Yres_buf,
                 self.estimates.Yr_buf,
                 self.estimates.rho_buf,
                 self.params.get('data', 'dims'),
                 self.params.get('init', 'gSig'),
                 self.params.get('init', 'gSiz'),
                 self.ind_A,
                 self.estimates.CY,
                 self.estimates.CC,
                 rval_thr=self.params.get('online', 'rval_thr'),
                 thresh_fitness_delta=self.params.get('online',
                                                      'thresh_fitness_delta'),
                 thresh_fitness_raw=self.params.get('online',
                                                    'thresh_fitness_raw'),
                 thresh_overlap=self.params.get('online', 'thresh_overlap'),
                 groups=self.estimates.groups,
                 batch_update_suff_stat=self.params.get(
                     'online', 'batch_update_suff_stat'),
                 gnb=self.params.get('init', 'nb'),
                 sn=self.estimates.sn,
                 g=(np.mean(self.estimates.g) if self.params.get(
                     'preprocess', 'p') == 1 else np.mean(self.estimates.g, 0)),
                 s_min=self.params.get('temporal', 's_min'),
                 Ab_dense=self.estimates.Ab_dense if self.params.get(
                     'online', 'use_dense') else None,
                 oases=self.estimates.OASISinstances if self.params.get(
                     'preprocess', 'p') else None,
                 N_samples_exceptionality=self.params.get(
                     'online', 'N_samples_exceptionality'),
            ...
            if num_added > 0:
                self.N += num_added
                self.M += num_added
                if self.N + self.params.get('online',
                                            'max_num_added') > expected_comps:
                    expected_comps += 200
                    self.params.set('online',
                    ...
                    if self.params.get(
                            'online', 'use_dense'
                    ):  # resize won't work due to contingency issue
                        # self.estimates.Ab_dense.resize([self.estimates.CY.shape[-1], expected_comps+nb_])
                        self.estimates.Ab_dense = np.zeros(
                            (self.estimates.CY.shape[-1], expected_comps + nb_),
                            dtype=np.float32)
                ...
                self.estimates.noisyC[self.M - num_added:self.M, t - mbs + 1:t +
                                      1] = Cf_temp[self.M - num_added:self.M]
                ...
                for _ct in range(self.M - num_added, self.M):
                    ...
                    if self.params.get('preprocess', 'p'):
                        ...
                        self.estimates.C_on[_ct, t - mbs + 1:t +
                                            1] = self.estimates.OASISinstances[
                                                _ct - nb_].get_c(mbs)
                    else:
                        self.estimates.C_on[_ct, t - mbs + 1:t +
                                            1] = np.maximum(
                                                0,
                                                self.estimates.noisyC[_ct, t -
                                                                      mbs +
                                                                      1:t + 1])
                    if self.params.get('online',
                                       'simultaneously') and self.params.get(
                                           'online', 'n_refit'):
                        self.estimates.AtY_buf = np.concatenate(
                            (self.estimates.AtY_buf, [
                                Ab_.data[Ab_.indptr[_ct]:Ab_.indptr[_ct + 1]].
                                dot(self.estimates.Yr_buf.T[
                                    Ab_.indices[Ab_.indptr[_ct]:Ab_.indptr[_ct +
                                                                           1]]])
                            ]))
                ...
                self.estimates.AtA = np.zeros((self.M, self.M),
                                              dtype=np.float32)
                ...
                self.estimates.AtA[
                    -num_added:] = self.estimates.AtA[:, -num_added:].T

                # set the update counter to 0 for components that are overlaping the newly added
                idx_overlap = self.estimates.AtA[nb_:-num_added,
                                                 -num_added:].nonzero()[0]
        ...
        if self.params.get('online', 'batch_update_suff_stat'):
            ...
            ccf = self.estimates.C_on[:self.M, t - self.params.
                                      get('online', 'minibatch_suff_stat'):t -
                                      self.params.
                                      get('online', 'minibatch_suff_stat') + 1]
            y = self.estimates.Yr_buf.get_last_frames(
                self.params.get('online', 'minibatch_suff_stat') + 1)[:1]
            # much faster: exploit that we only access CY[m, ind_pixels], hence update only these
            for m in range(self.N):
                self.estimates.CY[m + nb_, self.ind_A[m]] *= (1 - 1. / t)
                self.estimates.CY[m + nb_, self.ind_A[m]] += ccf[m + nb_].dot(
                    y[:, self.ind_A[m]]) / t
            self.estimates.CY[:nb_] = self.estimates.CY[:nb_] * (
                1 - 1. / t) + ccf[:nb_].dot(y / t)
            self.estimates.CC = self.estimates.CC * (1 - 1. / t) + ccf.dot(
                ccf.T / t)

        # update shapes
        t_sh = time()
        if not self.params.get('online',
                               'dist_shape_update'):  # bulk shape update
            if ((t + 1 - self.params.get('online', 'init_batch')) %
                    self.params.get('online', 'update_freq') == 0):
                print('Updating Shapes')

                if self.N > self.params.get('online', 'max_comp_update_shape'):
                    indicator_components = np.where(
                        self.update_counter <= self.params.get(
                            'online', 'num_times_comp_updated'))[0]
                ...
                ind_zero = list(
                    np.where(self.estimates.AtA.diagonal() < 1e-10)[0])
                if len(ind_zero) > 0:
                    ind_zero.sort()
                    ind_zero = ind_zero[::-1]
                    ...
                    self.estimates.AtA = np.delete(self.estimates.AtA,
                                                   ind_zero,
                                                   axis=0)
                    self.estimates.AtA = np.delete(self.estimates.AtA,
                                                   ind_zero,
                                                   axis=1)
                    self.estimates.CY = np.delete(self.estimates.CY,
                                                  ind_zero,
                                                  axis=0)
                    self.estimates.CC = np.delete(self.estimates.CC,
                                                  ind_zero,
                                                  axis=0)
                    self.estimates.CC = np.delete(self.estimates.CC,
                                                  ind_zero,
                    ...
                    self.M -= len(ind_zero)
                    self.N -= len(ind_zero)
                    self.estimates.noisyC = np.delete(self.estimates.noisyC,
                                                      ind_zero,
                                                      axis=0)
                    for ii in ind_zero:
                        del self.estimates.OASISinstances[
                            ii - self.params.get('init', 'nb')]
                        #del self.ind_A[ii-self.params.init['nb']]

                    self.estimates.C_on = np.delete(self.estimates.C_on,
                                                    ind_zero,
                                                    axis=0)
                    self.estimates.AtY_buf = np.delete(self.estimates.AtY_buf,
                                                       ind_zero,
                                                       axis=0)
                    #Ab_ = Ab_[:,ind_keep]
                    ...
                    self.ind_A = list([
                        (self.estimates.Ab.indices[self.estimates.Ab.indptr[ii]:
                                                   self.estimates.Ab.indptr[ii +
                                                                            1]])
                        for ii in range(self.params.get('init', 'nb'), self.M)
                    ])
                    self.estimates.groups = list(map(list,
                                                     update_order(Ab_)[0]))
            ...
            self.update_counter *= .5**(
                1. / self.params.get('online', 'update_freq'))
            # if not num_added:
            if (not num_added) and (
                    time() - t_start < 2 * self.time_spend /
                (t - self.params.get('online', 'init_batch') + 1)):
    ...
    def initialize_online(self):
        ...
        Y = caiman.load(fls[0],
                        subindices=slice(0, opts['init_batch'], None),
                        var_name_hdf5=self.params.get(
                            'data', 'var_name_hdf5')).astype(np.float32)
        ...
        if self.params.get('online', 'init_method') == 'bare':
            ...
            self.estimates.g = np.array([
                -np.poly([0.9] * max(self.params.get('preprocess', 'p'), 1))[1:]
...
#                cnm.estimates.f, cnm.estimates.S, self.estimates.YrA
                ...
                if self.params.get('online', 'normalize'):
                    self.estimates.A /= self.img_norm.reshape(
                        -1, order='F')[:, np.newaxis]
                    self.estimates.b /= self.img_norm.reshape(
                        -1, order='F')[:, np.newaxis]
    ...
    def fit_online(self, **kwargs):
        ...
        if self.params.get('online', 'save_online_movie'):
            ...
            out = cv2.VideoWriter(
                self.params.get('online', 'movie_name_online'), fourcc, 30.0,
                tuple([int(2 * x) for x in self.params.get('data', 'dims')]))
        ...
        #     Go through all files
            for file_count, ffll in enumerate(process_files):
                ...
                Y_ = caiman.load(ffll,
                                 var_name_hdf5=self.params.get(
                                     'data', 'var_name_hdf5'),
                                 subindices=slice(init_batc_iter[file_count],
                                                  None, None))
                ...
                for frame_count, frame in enumerate(Y_):  # process each file
                    ...
                    if t % 100 == 0:
                        print(
                            ...
                            + str(self.estimates.Ab.shape[-1] -
                                  self.params.get('init', 'nb')))
                        old_comps = self.N
                    ...
                    # Motion Correction
                    if self.params.get('online',
                                       'motion_correct'):  # motion correct
                        templ = self.estimates.Ab.dot(
                            self.estimates.C_on[:self.M, t - 1]).reshape(
                                self.params.get(
                                    'data', 'dims'), order='F') * self.img_norm
                        if self.params.get('motion', 'pw_rigid'):
                            ...
                            frame_cor, shift = motion_correct_iteration_fast(
                                frame_, templ, max_shifts_online,
                                max_shifts_online)
        ...
        if self.params.get('online', 'normalize'):
            self.estimates.Ab /= 1. / self.img_norm.reshape(
                -1, order='F')[:, np.newaxis]
            self.estimates.Ab = csc_matrix(self.estimates.Ab)
        self.estimates.A, self.estimates.b = self.estimates.Ab[:,
                                                               self.params.get(
                                                                   'init', 'nb'
                                                               ):], self.estimates.Ab[:, :self.params.get(
                                                                   'init', 'nb'
                                                               )].toarray()
        self.estimates.C, self.estimates.f = self.estimates.C_on[
            self.params.get('init', 'nb'):self.M, t - t //
            epochs:t], self.estimates.C_on[:self.params.get('init', 'nb'), t -
                                           t // epochs:t]
        noisyC = self.estimates.noisyC[self.params.get('init', 'nb'):self.M, t -
                                       t // epochs:t]
        ...
        if self.estimates.OASISinstances is not None:
            ...
            self.estimates.S = np.stack(
                [osi.s for osi in self.estimates.OASISinstances])
        ...
        if self.params.get('online', 'ds_factor') > 1:
            dims = Y_.shape[1:]
            self.estimates.A = hstack([
                coo_matrix(
                    cv2.resize(
                        self.estimates.A[:, i].reshape(self.dims,
                                                       order='F').toarray(),
                        dims[::-1]).reshape(-1, order='F')[:, None])
                for i in range(self.N)
            ],
                                      format='csc')
            self.estimates.b = np.concatenate([
                cv2.resize(self.estimates.b[:, i].reshape(self.dims, order='F'),
                           dims[::-1]).reshape(-1, order='F')[:, None]
                for i in range(self.params.get('init', 'nb'))
            ],
                                              axis=1)
            self.params.set('data', {'dims': dims})
    ...
    def create_frame(self, frame_cor, show_residuals=True, resize_fact=1):
        ...
        bgkrnd_frame = b.dot(f[:, self.t - 1]).reshape(
            self.dims, order='F')  # denoised frame (components + background)
        denoised_frame = comps_frame + bgkrnd_frame
        denoised_frame = (denoised_frame.copy() - self.bnd_Y[0]) / np.diff(
            self.bnd_Y)
        comps_frame = (comps_frame.copy() - self.bnd_AC[0]) / np.diff(
            self.bnd_AC)
        ...
        frame_comp_1 = cv2.resize(
            ...
            (2 * np.int(self.dims[1] * resize_fact),
             np.int(self.dims[0] * resize_fact)))
        ...
        cv2.putText(vid_frame,
                    ...
                    thickness=1)
        cv2.putText(vid_frame,
                    captions[1], (np.int(self.dims[0] * resize_fact) + 5, 20),
                    fontFace=5,
                    fontScale=0.8,
                    color=(0, 255, 0),
                    thickness=1)
        cv2.putText(vid_frame,
                    captions[2], (5, np.int(self.dims[1] * resize_fact) + 20),
                    fontFace=5,
                    fontScale=0.8,
                    color=(0, 255, 0),
                    thickness=1)
        cv2.putText(vid_frame,
                    captions[3], (np.int(self.dims[0] * resize_fact) + 5,
                                  np.int(self.dims[1] * resize_fact) + 20),
                    fontFace=5,
                    fontScale=0.8,
                    color=(0, 255, 0),
                    thickness=1)
...
def bare_initialization(Y,
    ...
    if return_object:
        ...
        cnm_init.estimates.g = np.array(
            [-np.poly([0.9, 0.5][:max(1, p)])[1:] for gg in np.ones(k)])
...
def seeded_initialization(Y,
    ...
    if order_init is not None:  #initialize using rank-1 nmf for each component
        ...
        for count, idx_in in enumerate(order_init):
            ...
            Yr_no_bg[idx_domain] -= np.outer(Ain[idx_domain, idx_in],
                                             Cin[idx_in])
    else:
    ...
    YrA = YA - AA.T.dot(Cin)
    if return_object:
        cnm_init = caiman.source_extraction.cnmf.cnmf.CNMF(2,
                                                           Ain=Ain,
                                                           Cin=Cin,
                                                           b_in=np.array(b_in),
                                                           f_in=f_in,
        ...
        cnm_init.estimates.g = np.array(
            [-np.poly([0.9] * max(p, 1))[1:] for gg in np.ones(nr)])
    ...
    for _ in range(iters):
        for m in range(K):  # neurons
            ...
            A[ind_pixels, m] = np.clip(
                A[ind_pixels, m] +
                ((U[m, ind_pixels] - V[m].dot(A[ind_pixels].T)) / V_diag[m]), 0,
                np.inf)
...
def HALS4activity(Yr,
    ...
    def norm(c):
        return sqrt(c.ravel().dot(c.ravel()))
...
def demix_and_deconvolve(C,
    ...
    if n_refit == 0:
        ...
        def refit(o, c):
            # remove last pools
            tmp = 0
            while tmp < n_refit and o.t - o.get_l_of_last_pool(
            ) > T - len_buffer:
                o.remove_last_pool()
                tmp += 1
            # refit last pools
...
def init_shapes_and_sufficient_stats(Y, A, C, b, f, bSiz=3):
    ...
    A_smooth = np.transpose([
        gaussian_filter(np.array(a).reshape(dims, order='F'),
                        0).ravel(order='F') for a in Ab.T
    ])
    ...
    Ab = csc_matrix(Ab)
    ind_A = [
        Ab.indices[Ab.indptr[m]:Ab.indptr[m + 1]] for m in range(nb, nb + K)
    ]
...
def update_shapes(CY,
    ...
    if sn is None or q == 0.5:  # avoid costly construction of L=np.zeros((M, D), dtype=np.float32)
        for _ in range(
                iters
        ):  # it's presumably better to run just 1 iter but update more neurons
            if Ab_dense is None:
                for m in idx_comp:  # neurons
                    ...
                    if tmp.dot(tmp) > 0:
                        ...
                        ind_A[m - nb] = Ab.indices[slice(
                            Ab.indptr[m], Ab.indptr[m + 1])]
            else:
                for m in idx_comp:  # neurons
                    ind_pixels = ind_A[m - nb]
                    tmp = np.maximum(
                    ...
                    if tmp.dot(tmp) > 0:
                        tmp *= 1e-3 / \
                            min(1e-3, sqrt(tmp.dot(tmp)) + np.finfo(float).eps)
                        Ab_dense[ind_pixels, m] = tmp / max(
                            1, sqrt(tmp.dot(tmp)))
                        Ab.data[Ab.indptr[m]:Ab.
                                indptr[m + 1]] = Ab_dense[ind_pixels, m]
                        ind_A[m - nb] = Ab.indices[slice(
                            Ab.indptr[m], Ab.indptr[m + 1])]
            if update_bkgrd:
        ...
        for _ in range(
            ...
            if Ab_dense is None:
                for m in idx_comp:  # neurons
                    ...
                    tmp = np.maximum(
                        Ab.data[Ab.indptr[m]:Ab.indptr[m + 1]] +
                        ((CY[m, ind_pixels] - L[m, ind_pixels] -
                          Ab.dot(CC[m])[ind_pixels]) / CC[m, m]), 0)
                ...
                for m in idx_comp:  # neurons
                    ...
                    tmp = np.maximum(
                        Ab_dense[ind_pixels, m] +
                        ((CY[m, ind_pixels] - L[m, ind_pixels] -
                          Ab_dense[ind_pixels].dot(CC[m])) / CC[m, m]), 0)
                    # normalize
                    if tmp.dot(tmp) > 0:
                        tmp *= 1e-3 / \
                            min(1e-3, sqrt(tmp.dot(tmp)) + np.finfo(float).eps)
    ...
    def get_last_frames(self, num_frames):
        if self.cur >= num_frames:
            ...
            return np.concatenate(
                [self[(self.cur - num_frames):], self[:self.cur]], axis=0)
...
#%%
@profile
def get_candidate_components(sv,
                             dims,
                             Yres_buf,
                             min_num_trial=3,
                             gSig=(5, 5),
    ...
    if use_peak_max:
        ...
        thresh_img_sel = np.median(
            img_select_peaks) + thresh_std_peak_resid * np.std(img_select_peaks)
        ...
        local_maxima = peak_local_max(img_select_peaks,
                                      ...
                                      num_peaks=min_num_trial,
                                      threshold_abs=thresh_img_sel,
    ...
    for i in range(min_num_trial):
        ...
        ij = [
            min(max(ij_val, g_val), dim_val - g_val - 1)
        ...
        indices = np.ravel_multi_index(
            np.ix_(*[np.arange(ij[0], ij[1]) for ij in ijSig]), dims,
            order='F').ravel(order='C')
        ...
        if sniper_mode:
            half_crop_cnn = tuple(
                [int(np.minimum(gs * 2, patch_size / 2)) for gs in gSig])
            ij_cnn = [
                min(max(ij_val, g_val), dim_val - g_val - 1)
                for ij_val, g_val, dim_val in zip(ij, half_crop_cnn, dims)
            ]
            ijSig_cnn = [[max(i - g, 0), min(i + g + 1, d)]
                         for i, g, d in zip(ij_cnn, half_crop_cnn, dims)]
            indices_cnn = np.ravel_multi_index(
                np.ix_(*[np.arange(ij[0], ij[1]) for ij in ijSig_cnn]),
                dims,
                order='F').ravel(order='C')
    ...
    if sniper_mode & (len(Ain_cnn) > 0):
        ...
        Ain2 = np.reshape(Ain2, (-1,) + tuple(np.diff(ijSig_cnn).squeeze()),
                          order='F')
        Ain2 = np.stack(
            [cv2.resize(ain, (patch_size, patch_size)) for ain in Ain2])
        if tf_in is None:
            predictions = loaded_model.predict(Ain2[:, :, :, np.newaxis],
                                               batch_size=min_num_trial,
                                               verbose=0)
        else:
            predictions = loaded_model.run(
                tf_out, feed_dict={tf_in: Ain2[:, :, :, np.newaxis]})
...
def update_num_components(t,
    ...
    for ind, ain, cin, cin_res in zip(inds, Ains, Cins, Cins_res):
        ...
        if Ab_dense is None:
            ...
            ff = np.where(
                Ab_dense[indices, gnb:M].T.dot(ain).T > thresh_overlap)[0] + gnb
        ...
        # use s_min * noise estimate * sqrt(1-sum(gamma))
            ...
            s_min = -s_min * sqrt(
                (ain**2).dot(sn[indices]**2)) * sqrt(1 - np.sum(g))
        ...
        if accepted:
            ...
            Ain_csc = csc_matrix((ain, (indices, [0] * len(indices))),
                                 (np.prod(dims), 1),
                                 dtype=np.float32)
            if Ab_dense is None:
            ...
            slices_update = tuple(
                slice(max(0, ijs[0] - sg // 2), min(d, ijs[1] + sg // 2))
                for ijs, sg, d in zip(ijSig, gSiz, dims))
            # filter even bigger region to avoid boundary effects
            slices_filter = tuple(
                slice(max(0, ijs[0] - sg), min(d, ijs[1] + sg))
                for ijs, sg, d in zip(ijSig, gSiz, dims))

            ind_vb = np.ravel_multi_index(
                np.ix_(*[np.arange(sl.start, sl.stop) for sl in slices_update]),
                dims,
                order='C').ravel()

            if len(dims) == 3:
                rho_buf[:, ind_vb] = np.stack([
                    imblur(
                        vb.reshape(dims, order='F')[slices_filter],
                        sig=gSig,
                        siz=gSiz,
                        nDimBlur=len(dims))[tuple([
                            slice(
                ...
                Y_filter = Yres_buf.reshape(
                    (-1,) + dims,
                    order='F')[:, slices_filter[0], slices_filter[1]]
                T, d0, d1 = Y_filter.shape
                tmp = np.concatenate(
                    (Y_filter, np.zeros((T, gHalf[0], d1), dtype=np.float32)),
                    axis=1).reshape(-1, d1)
                cv2.GaussianBlur(tmp, tuple(gSiz), gSig[0], tmp, gSig[1],
                                 cv2.BORDER_CONSTANT)
                slices = tuple([
                ...
                rho_buf[:, ind_vb] = tmp.reshape(
                    T, -1, d1)[(slice(None),) + slices].reshape(T, -1)**2
...
def remove_components_online(ind_rem, gnb, Ab, use_dense, Ab_dense, AtA, CY, CC,
                             M, N, noisyC, OASISinstances, C_on, exp_comps):
    """
...
def initialize_movie_online(Y,
    ...
    #    pl.close('all')
    cnm_init = caiman.source_extraction.cnmf.CNMF(
        n_processes,
        method_init='greedy_roi',
        k=K,
        gSig=gSig,
    ...
    #%
    cnm_refine = caiman.source_extraction.cnmf.CNMF(
        n_processes,
        method_init='greedy_roi',
        k=A_tot.shape,
        gSig=gSig,
