...
class OnACID(object):
    ...
    def _prepare_object(self, Yr, T, new_dims=None, idx_components=None):
        ...
        if new_dims is not None:
            ...
            for ffrr in range(T):
                tmp = cv2.resize(
                    Yr[:, ffrr].reshape(self.dims, order="F"), new_dims[::-1]
                )
                print(tmp.shape)
            ...
            self.estimates.b = cv2.resize(self.estimates.b, new_dims[::-1]).reshape(
                [-1, 1], order="F"
            )
        ...
        self.estimates.noisyC[
            self.params.get("init", "nb") : self.M,
            : self.params.get("online", "init_batch"),
        ] = (self.estimates.C + self.estimates.YrA)
        self.estimates.noisyC[
            : self.params.get("init", "nb"), : self.params.get("online", "init_batch")
        ] = self.estimates.f
        ...
        if self.params.get("preprocess", "p"):
            ...
            self.estimates.OASISinstances = [
                OASIS(
                    g=np.ravel(0.01)
                    if self.params.get("preprocess", "p") == 0
                    else gam[0],
                    ...
                    if use_L1
                        ...
                        if self.params.get("temporal", "s_min") > 0
                        else (
                            -self.params.get("temporal", "s_min")
                            * sn
            ...
            self.estimates.C_on[
                : self.N, : self.params.get("online", "init_batch")
            ] = self.estimates.C
        ...
        self.estimates.Ab, self.ind_A, self.estimates.CY, self.estimates.CC = init_shapes_and_sufficient_stats(
            Yr[:, : self.params.get("online", "init_batch")].reshape(
                self.params.get("data", "dims") + (-1,), order="F"
            ),
            self.estimates.A,
            self.estimates.C_on[: self.N, : self.params.get("online", "init_batch")],
            self.estimates.b,
            self.estimates.noisyC[
        ...
        self.estimates.CY, self.estimates.CC = (
            self.estimates.CY * 1.0 / self.params.get("online", "init_batch"),
            1 * self.estimates.CC / self.params.get("online", "init_batch"),
        )
        ...
        if self.params.get("online", "use_dense"):
            ...
            self.estimates.Ab_dense[
                :, : self.estimates.Ab.shape[1]
        ...
        self.params.set(
            "init",
            {
                "gSiz": np.add(
                    np.multiply(
                        np.ceil(self.params.get("init", "gSig")).astype(np.int), 2
                    ),
                    1,
                )
            },
        ...
        self.estimates.Yr_buf = RingBuffer(
            Yr[
                :,
                self.params.get("online", "init_batch")
                - self.params.get("online", "minibatch_shape") : self.params.get(
                    "online", "init_batch"
                ),
            ].T.copy(),
        ...
        self.estimates.rho_buf = (
            imblur(
                np.maximum(self.estimates.Yres_buf.T, 0).reshape(
                    self.params.get("data", "dims") + (-1,), order="F"
                ),
                sig=self.params.get("init", "gSig"),
                siz=self.params.get("init", "gSiz"),
                nDimBlur=len(self.params.get("data", "dims")),
            )
            ** 2
        )
        self.estimates.rho_buf = np.reshape(
            self.estimates.rho_buf, (np.prod(self.params.get("data", "dims")), -1)
        ).T
        self.estimates.rho_buf = np.ascontiguousarray(self.estimates.rho_buf)
        self.estimates.rho_buf = RingBuffer(
            self.estimates.rho_buf, self.params.get("online", "minibatch_shape")
        )
        ...
        if (
            self.params.get("online", "path_to_model") is None
            or self.params.get("online", "sniper_mode") is False
        ):
            loaded_model = None
    ...
    def fit_next(self, t, frame_in, num_iters_hals=3):
        ...
        if (not self.params.get("online", "simultaneously")) or self.params.get(
            ...
            self.estimates.C_on[: self.M, t], self.estimates.noisyC[
                : self.M, t
            ] = HALS4activity(
                frame,
                self.estimates.Ab,
            ...
            if self.params.get("preprocess", "p"):
                ...
                for i, o in enumerate(self.estimates.OASISinstances):
                    ...
                    self.estimates.C_on[
                        nb_ + i, t - o.get_l_of_last_pool() + 1 : t + 1
                    ] = o.get_c_of_last_pool()
            ...
            self.estimates.AtY_buf = (
                np.concatenate(
                    (
                        self.estimates.AtY_buf[:, 1:],
                        self.estimates.Ab.T.dot(frame)[:, None],
                    ),
                    1,
                )
                if self.params.get("online", "n_refit")
                else self.estimates.Ab.T.dot(frame)[:, None]
            )
        ...
        self.estimates.vr = (t - 1) / t * self.estimates.vr + (res_frame - mn_) * (
            res_frame - self.estimates.mn
        ) / t
        ...
        if self.params.get("online", "update_num_comps"):
            ...
            res_frame = np.reshape(
                res_frame, self.params.get("data", "dims"), order="F"
            )

            rho = (
                imblur(
                    np.maximum(res_frame, 0),
                    sig=self.params.get("init", "gSig"),
                    siz=self.params.get("init", "gSiz"),
                    nDimBlur=len(self.params.get("data", "dims")),
                )
            ...
            (
                ...
                g=(
                    np.mean(self.estimates.g)
                    if self.params.get("preprocess", "p") == 1
                    else np.mean(self.estimates.g, 0)
                ),
                s_min=self.params.get("temporal", "s_min"),
                Ab_dense=self.estimates.Ab_dense
                if self.params.get("online", "use_dense")
                else None,
                oases=self.estimates.OASISinstances
                if self.params.get("preprocess", "p")
                else None,
            ...
            if num_added > 0:
                ...
                if self.N + self.params.get("online", "max_num_added") > expected_comps:
                    ...
                    print(
                        "Increasing number of expected components to:"
                        + str(expected_comps)
                    )
                self.update_counter.resize(self.N)

                self.estimates.noisyC[
                    self.M - num_added : self.M, t - mbs + 1 : t + 1
                ] = Cf_temp[self.M - num_added : self.M]
                ...
                for _ct in range(self.M - num_added, self.M):
                    ...
                    if self.params.get("preprocess", "p"):
                        ...
                        self.estimates.C_on[
                            _ct, t - mbs + 1 : t + 1
                        ] = self.estimates.OASISinstances[_ct - nb_].get_c(mbs)
                    else:
                        self.estimates.C_on[_ct, t - mbs + 1 : t + 1] = np.maximum(
                            0, self.estimates.noisyC[_ct, t - mbs + 1 : t + 1]
                        )
                    if self.params.get("online", "simultaneously") and self.params.get(
                        "online", "n_refit"
                    ):
                        self.estimates.AtY_buf = np.concatenate(
                            (
                                self.estimates.AtY_buf,
                                [
                                    Ab_.data[Ab_.indptr[_ct] : Ab_.indptr[_ct + 1]].dot(
                ...
                idx_overlap = self.estimates.AtA[nb_:-num_added, -num_added:].nonzero()[
                    0
                ]
        ...
        if self.params.get("online", "batch_update_suff_stat"):
            ...
            if (t + 1 - self.params.get("online", "init_batch")) % min_batch == 0:
                ...
                w1 = (t - n0 + t0) * 1.0 / (t + t0)  # (1 - 1./t)#mbs*1. / t
                w2 = 1.0 / (t + t0)  # 1.*mbs /t
                for m in range(self.N):
                    self.estimates.CY[m + nb_, self.ind_A[m]] *= w1
                    self.estimates.CY[m + nb_, self.ind_A[m]] += w2 * ccf[m + nb_].dot(
                        y[:, self.ind_A[m]]
                    )

                self.estimates.CY[:nb_] = self.estimates.CY[:nb_] * w1 + w2 * ccf[
                    :nb_
                ].dot(
                    y
                )  # background
            ...
            ccf = self.estimates.C_on[
                : self.M,
                t
                - self.params.get("online", "minibatch_suff_stat") : t
                - self.params.get("online", "minibatch_suff_stat")
                + 1,
            ]
            ...
            # much faster: exploit that we only access CY[m, ind_pixels], hence update only these
            for m in range(self.N):
                self.estimates.CY[m + nb_, self.ind_A[m]] *= 1 - 1.0 / t
                self.estimates.CY[m + nb_, self.ind_A[m]] += (
                    ccf[m + nb_].dot(y[:, self.ind_A[m]]) / t
                )
            self.estimates.CY[:nb_] = self.estimates.CY[:nb_] * (1 - 1.0 / t) + ccf[
                :nb_
            ].dot(y / t)
            self.estimates.CC = self.estimates.CC * (1 - 1.0 / t) + ccf.dot(ccf.T / t)

        # update shapes
        t_sh = time()
        if not self.params.get("online", "dist_shape_update"):  # bulk shape update
            if (t + 1 - self.params.get("online", "init_batch")) % self.params.get(
                "online", "update_freq"
            ) == 0:
                ...
                if self.N > self.params.get("online", "max_comp_update_shape"):
                    indicator_components = np.where(
                        self.update_counter
                        <= self.params.get("online", "num_times_comp_updated")
                    )[0]
                ...
                if self.params.get("online", "use_dense"):
                    ...
                    Ab_, self.ind_A, self.estimates.Ab_dense[
                        :, : self.M
                    ] = update_shapes(
                        self.estimates.CY,
                        self.estimates.CC,
                ...
                if len(ind_zero) > 0:
                    ...
                    self.M -= len(ind_zero)
                    self.N -= len(ind_zero)
                    self.estimates.noisyC = np.delete(
                        self.estimates.noisyC, ind_zero, axis=0
                    )
                    for ii in ind_zero:
                    ...
                    self.estimates.C_on = np.delete(
                        self.estimates.C_on, ind_zero, axis=0
                    )
                    self.estimates.AtY_buf = np.delete(
                        self.estimates.AtY_buf, ind_zero, axis=0
                    )
                    # Ab_ = Ab_[:,ind_keep]
                    ...
                    self.ind_A = list(
                        [
                            (
                                self.estimates.Ab.indices[
                                    self.estimates.Ab.indptr[
                                        ii
                                    ] : self.estimates.Ab.indptr[ii + 1]
                                ]
                            )
            ...
            self.update_counter *= 0.5 ** (
                1.0 / self.params.get("online", "update_freq")
            )
            # if not num_added:
                ...
                if len(candidates):
                    ...
                    if self.params.get("online", "use_dense"):
                        ...
                        if update_bkgrd:
                            ...
                            self.estimates.AtA[
                                indicator_components, indicator_components[:, None]
    ...
    def initialize_online(self):
        ...
        if ds_factor > 1:
            Y = Y.resize(1.0 / ds_factor, 1.0 / ds_factor)
                ...
                n_p = len(
                    [
                        ...
                        for it in sliding_window(
                            Y[0],
                            self.params.get("motion", "overlaps"),
        ...
        if self.params.get("online", "init_method") == "bare":
            ...
            #                cnm.estimates.f, cnm.estimates.S, self.estimates.YrA
                ...
                if self.params.get("online", "normalize"):
                    self.estimates.A /= self.img_norm.reshape(-1, order="F")[
                        :, np.newaxis
                    ]
                    self.estimates.b /= self.img_norm.reshape(-1, order="F")[
                        :, np.newaxis
                    ]
        ...
        if opts["show_movie"]:
            self.bnd_AC = np.percentile(
                self.estimates.A.dot(self.estimates.C), (0.001, 100 - 0.005)
            )
            self.bnd_BG = np.percentile(
                self.estimates.b.dot(self.estimates.f), (0.001, 100 - 0.001)
            )
        return self
    ...
    def fit_online(self, **kwargs):
        ...
        if self.params.get("online", "save_online_movie"):
            ...
            out = cv2.VideoWriter(
                self.params.get("online", "movie_name_online"),
                fourcc,
                30.0,
            ...
            for file_count, ffll in enumerate(process_files):
                ...
                for frame_count, frame in enumerate(Y_):  # process each file
                    ...
                    if self.params.get("online", "motion_correct"):  # motion correct
                        templ = (
                            self.estimates.Ab.dot(
                                self.estimates.C_on[: self.M, t - 1]
                            ).reshape(self.params.get("data", "dims"), order="F")
                            * self.img_norm
                        )
                        if self.params.get("motion", "pw_rigid"):
                            frame_cor, shift, _, xy_grid = tile_and_correct(
                                ...
                                show_movie=False,
                                max_deviation_rigid=self.params.motion[
                                    "max_deviation_rigid"
                                ],
                                add_to_movie=0,
                                shifts_opencv=True,
        ...
        if self.params.get("online", "normalize"):
            self.estimates.Ab /= (
                1.0 / self.img_norm.reshape(-1, order="F")[:, np.newaxis]
            )
            self.estimates.Ab = csc_matrix(self.estimates.Ab)
        self.estimates.A, self.estimates.b = (
            self.estimates.Ab[:, self.params.get("init", "nb") :],
            self.estimates.Ab[:, : self.params.get("init", "nb")].toarray(),
        )
        self.estimates.C, self.estimates.f = (
            self.estimates.C_on[
                self.params.get("init", "nb") : self.M, t - t // epochs : t
            ],
            self.estimates.C_on[: self.params.get("init", "nb"), t - t // epochs : t],
        )
        noisyC = self.estimates.noisyC[
            self.params.get("init", "nb") : self.M, t - t // epochs : t
        ]
        ...
        if self.params.get("online", "ds_factor") > 1:
            ...
            self.estimates.A = hstack(
                [
                    coo_matrix(
                        cv2.resize(
                            self.estimates.A[:, i]
                            .reshape(self.dims, order="F")
                            .toarray(),
                            dims[::-1],
                        ).reshape(-1, order="F")[:, None]
                    )
            ...
            self.estimates.b = np.concatenate(
                [
                    cv2.resize(
                        self.estimates.b[:, i].reshape(self.dims, order="F"), dims[::-1]
                    ).reshape(-1, order="F")[:, None]
    ...
    def create_frame(self, frame_cor, show_residuals=True, resize_fact=1):
        ...
        frame_comp_1 = cv2.resize(
            np.concatenate([frame_plot, all_comps * 1.0], axis=-1),
            (
        ...
        vid_frame = np.minimum((vid_frame * 255.0), 255).astype("u1")
        ...
        cv2.putText(
            vid_frame,
            captions[1],
            (np.int(self.dims[0] * resize_fact) + 5, 20),
            fontFace=5,
            fontScale=0.8,
            color=(0, 255, 0),
            thickness=1,
        ...
        cv2.putText(
            vid_frame,
            captions[2],
            (5, np.int(self.dims[1] * resize_fact) + 20),
            fontFace=5,
            fontScale=0.8,
            color=(0, 255, 0),
            thickness=1,
        ...
        cv2.putText(
            ...
            captions[3],
            (
                np.int(self.dims[0] * resize_fact) + 5,
...
def bare_initialization(
    ...
    **kwargs
    ...
    YA = spdiags(old_div(1.0, nA), 0, nr, nr) * (
        Ain.T.dot(Yr) - (Ain.T.dot(b_in)).dot(f_in)
    )
    AA = spdiags(old_div(1.0, nA), 0, nr, nr) * (Ain.T.dot(Ain))
    YrA = YA - AA.T.dot(Cin)
    if return_object:
        ...
        cnm_init.estimates.A, cnm_init.estimates.C, cnm_init.estimates.b, cnm_init.estimates.f, cnm_init.estimates.S, cnm_init.estimates.YrA = (
            Ain,
            Cin,
            b_in,
            f_in,
            np.maximum(np.atleast_2d(Cin), 0),
            YrA,
        )
    ...
    if order_init is not None:  # initialize using rank-1 nmf for each component
        ...
        for count, idx_in in enumerate(order_init):
            ...
            Ain[idx_domain, idx_in] = model_comp.fit_transform(
                np.maximum(Yr_no_bg[idx_domain], 0)
            ).squeeze()
...
@profile
def HALS4activity(Yr, A, noisyC, AtA=None, iters=5, tol=1e-3, groups=None, order=None):
    ...
    if n_refit == 0:
        for i in range(iters):
            for m in range(AtY.shape[0]):
                noisyC[m, -1] = (
                    C[m, -1] + (AtY[m, -1] - AtA[m].dot(C[:, -1])) / AtA[m, m]
                )
                if m >= nb and i > 0:
                    n = m - nb
                    if i == iters - 1:  # commit
        ...
        overlap = np.sum(AtA[nb:, nb:] > 0.1, 0) > 1
...
def init_shapes_and_sufficient_stats(Y, A, C, b, f, bSiz=3):
    ...
    A_smooth = np.transpose(
        [
            gaussian_filter(np.array(a).reshape(dims, order="F"), 0).ravel(order="F")
            for a in Ab.T
        ]
    )
...
def update_shapes(
    ...
    if (
        sn is None or q == 0.5
    ):  # avoid costly construction of L=np.zeros((M, D), dtype=np.float32)
        for _ in range(
            iters
            ...
            if Ab_dense is None:
                for m in idx_comp:  # neurons
                    ...
                    tmp = np.maximum(
                        ...
                        + ((CY[m, ind_pixels] - Ab.dot(CC[m])[ind_pixels]) / CC[m, m]),
                        0,
                    )
                    # normalize
                    if tmp.dot(tmp) > 0:
                        tmp *= 1e-3 / min(
                            1e-3, sqrt(tmp.dot(tmp)) + np.finfo(float).eps
                        )
                        tmp = tmp / max(1, sqrt(tmp.dot(tmp)))
                        Ab.data[Ab.indptr[m] : Ab.indptr[m + 1]] = tmp
                        ind_A[m - nb] = Ab.indices[
                            slice(Ab.indptr[m], Ab.indptr[m + 1])
                        ]
            else:
                for m in idx_comp:  # neurons
                    ind_pixels = ind_A[m - nb]
                    ...
                    if tmp.dot(tmp) > 0:
                        ...
                        Ab.data[Ab.indptr[m] : Ab.indptr[m + 1]] = Ab_dense[
                            ind_pixels, m
                        ]
                        ind_A[m - nb] = Ab.indices[
                            slice(Ab.indptr[m], Ab.indptr[m + 1])
...
def get_candidate_components(
    ...
    if use_peak_max:
        ...
        img_select_peaks = cv2.GaussianBlur(
            img_select_peaks,
            ksize=ksize,
            sigmaX=gSig[0],
            sigmaY=gSig[1],
            ...
            img_select_peaks, ddepth=-1, ksize=ksize, borderType=cv2.BORDER_REPLICATE
        )
        thresh_img_sel = np.median(img_select_peaks) + thresh_std_peak_resid * np.std(
            img_select_peaks
        )
    ...
    for i in range(min_num_trial):
        ...
        indices = np.ravel_multi_index(
            np.ix_(*[np.arange(ij[0], ij[1]) for ij in ijSig]), dims, order="F"
        ).ravel(order="C")
        ...
        if sniper_mode:
            ...
            ijSig_cnn = [
                [max(i - g, 0), min(i + g + 1, d)]
                for i, g, d in zip(ij_cnn, half_crop_cnn, dims)
            ]
            indices_cnn = np.ravel_multi_index(
                np.ix_(*[np.arange(ij[0], ij[1]) for ij in ijSig_cnn]), dims, order="F"
            ).ravel(order="C")
    ...
    if sniper_mode & (len(Ain_cnn) > 0):
        ...
        if tf_in is None:
            predictions = loaded_model.predict(
                Ain2[:, :, :, np.newaxis], batch_size=min_num_trial, verbose=0
            )
        else:
...
def update_num_components(
    ...
    for ind, ain, cin, cin_res in zip(inds, Ains, Cins, Cins_res):
        ...
        ijSig = [
            [max(i - temp_g, 0), min(i + temp_g + 1, d)]
        ...
        if Ab_dense is None:
            ...
            ff = (
                np.where(Ab_dense[indices, gnb:M].T.dot(ain).T > thresh_overlap)[0]
                + gnb
            )
        ...
        if ff.size > 0:
            ...
            if np.any(np.array(cc) > 0.25) and accepted:
        ...
        # use s_min * noise estimate * sqrt(1-sum(gamma))
            ...
            s_min = (
                -s_min * sqrt((ain ** 2).dot(sn[indices] ** 2)) * sqrt(1 - np.sum(g))
            )

        cin_res = cin_res.get_ordered()
        if accepted:
            if useOASIS:
                oas = OASIS(g=g, s_min=s_min, num_empty_samples=t + 1 - len(cin_res))
                for yt in cin_res:
                    oas.fit_next(yt)
        ...
        if accepted:
            ...
            if len(dims) == 3:
                ...
                tmp = np.concatenate(
                    (Y_filter, np.zeros((T, gHalf[0], d1), dtype=np.float32)), axis=1
                ).reshape(-1, d1)
                cv2.GaussianBlur(
                    tmp, tuple(gSiz), gSig[0], tmp, gSig[1], cv2.BORDER_CONSTANT
                )
                slices = tuple(
                    [
                        slice(
                ...
                rho_buf[:, ind_vb] = (
                    tmp.reshape(T, -1, d1)[(slice(None),) + slices].reshape(T, -1) ** 2
                )
