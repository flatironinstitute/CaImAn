...
#                                 Loading Object From File")
    ...
    def _prepare_object(self, Yr, T, new_dims=None, idx_components=None):
        ...
        if self.params.get('preprocess', 'p'):
            ...
            for i, o in enumerate(self.estimates.OASISinstances):
                ...
                self.estimates.C_on[i, :self.params.
                                    get('online', 'init_batch')] = o.c
        else:
        ...
        self.params.set(
            'init', {
                ...
                np.add(
                    np.multiply(
                        np.ceil(self.params.get('init', 'gSig')).astype(
                            np.int), 2), 1)
            })
    ...
    def fit_next(self, t, frame_in, num_iters_hals=3):
        ...
        if (not self.params.get('online',
                                'simultaneously')) or self.params.get(
                                    'preprocess', 'p') == 0:
            ...
            self.estimates.C_on[:self.M,
                                t], self.estimates.noisyC[:self.M,
                                                          t] = HALS4activity(
                                                              frame,
                                                              self.estimates.
                                                              Ab,
        ...
        if self.params.get('online', 'update_num_comps'):
            ...
            self.estimates.mean_buff += (res_frame - self.estimates.Yres_buf[
                self.estimates.Yres_buf.cur]) / self.params.get(
                    'online', 'minibatch_shape')
            ...
            (self.estimates.Ab, Cf_temp, self.estimates.Yres_buf,
             ...
             self.cnn_pos) = update_num_components(
                 ...
                 g=(np.mean(self.estimates.g)
                    if self.params.get('preprocess', 'p') == 1 else np.mean(
                        self.estimates.g, 0)),
                 ...
                 thresh_CNN_noisy=self.params.get('online',
                                                  'thresh_CNN_noisy'),
            ...
            if num_added > 0:
                    ...
                    if self.params.get('online',
                        ...
                        self.estimates.AtY_buf = np.concatenate(
                            (self.estimates.AtY_buf, [
                                ...
                                dot(self.estimates.Yr_buf.T[Ab_.indices[
                                    Ab_.indptr[_ct]:Ab_.indptr[_ct + 1]]])
                            ]))
                ...
                if self.params.get('online', 'use_dense'):
                    self.estimates.AtA[:,
                                       -num_added:] = self.estimates.Ab.T.dot(
                                           self.estimates.
                                           Ab_dense[:, self.M -
                                                    num_added:self.M])
                else:
        ...
        if self.params.get('online', 'batch_update_suff_stat'):
            ...
            if ((t + 1 - self.params.get('online', 'init_batch')) %
                ...
                self.estimates.CC = self.estimates.CC * w1 + w2 * ccf.dot(
                    ccf.T)
        ...
        if not self.params.get('online',
            ...
            if ((t + 1 - self.params.get('online', 'init_batch')) %
                ...
                if len(ind_zero) > 0:
                    ...
                    self.ind_A = list([
                        (self.estimates.Ab.
                         indices[self.estimates.Ab.indptr[ii]:self.estimates.
                                 Ab.indptr[ii + 1]])
    ...
    def fit_online(self, **kwargs):
        ...
        self.estimates.C, self.estimates.f = self.estimates.C_on[
            ...
            epochs:t], self.estimates.C_on[:self.params.get('init', 'nb'), t -
                                           t // epochs:t]
        noisyC = self.estimates.noisyC[self.params.get('init', 'nb'):self.
                                       M, t - t // epochs:t]
        self.estimates.YrA = noisyC - self.estimates.C
        if self.estimates.OASISinstances is not None:
            self.estimates.bl = [
                osi.b for osi in self.estimates.OASISinstances
            ]
        ...
        if self.params.get('online', 'ds_factor') > 1:
            ...
            self.estimates.b = np.concatenate([
                cv2.resize(
                    self.estimates.b[:, i].reshape(self.dims, order='F'),
                    dims[::-1]).reshape(-1, order='F')[:, None]
    ...
    for _ in range(iters):
        for m in range(K):  # neurons
            ...
            A[ind_pixels, m] = np.clip(
                A[ind_pixels, m] +
                ((U[m, ind_pixels] - V[m].dot(A[ind_pixels].T)) / V_diag[m]),
                0, np.inf)
...
def get_candidate_components(sv,
    ...
    if use_peak_max:
        ...
        thresh_img_sel = np.median(
            img_select_peaks
        ) + thresh_std_peak_resid * np.std(img_select_peaks)
...
def update_num_components(t,
    ...
    for ind, ain, cin, cin_res in zip(inds, Ains, Cins, Cins_res):
        ...
        if Ab_dense is None:
            ...
            ff = np.where(Ab_dense[indices, gnb:M].T.dot(ain).T >
                          thresh_overlap)[0] + gnb
                ...
                rho_buf[:, ind_vb] = np.stack([
                    imblur(
                        ...
                        nDimBlur=len(dims))[tuple([
                            slice(
                                slices_update[i].start -
                                slices_filter[i].start,
                                slices_update[i].stop - slices_filter[i].start)
                            for i in range(len(dims))
                        ])].ravel() for vb in Yres_buf
                ])**2
            else:
...
def remove_components_online(ind_rem, gnb, Ab, use_dense, Ab_dense, AtA, CY,
                             CC, M, N, noisyC, OASISinstances, C_on,
                             exp_comps):
    """
    ...
    ind_A = list([(Ab.indices[Ab.indptr[ii]:Ab.indptr[ii + 1]])
                  for ii in range(gnb, M)])
