...
class movie(ts.timeseries):
    ...
    def bin_median_3d(self, window=10):
        ...
        Args:
            mat: ndarray
                input 4D matrix, time along second dimension
        ...
        """
        d1, T, d2, d3 = np.shape(self)
        num_windows = np.int(old_div(T, window))
        num_frames = num_windows * window
        return np.nanmedian(np.nanmean(np.reshape(self[:,:num_frames], (d1, window, num_windows, d2, d3)), axis=1), axis=1)
