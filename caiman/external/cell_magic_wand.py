...
def image_cart_to_polar(image, center, min_radius, max_radius, phase_width, zoom_factor=1):
    ...
    theta, r = np.meshgrid(np.linspace(0, 2 * np.pi, phase_width), np.arange(min_radius, max_radius))
...
def mask_polar_to_cart(mask, center, min_radius, max_radius, output_shape, zoom_factor=1):
    ...
    # coordinate conversion
    theta, r = np.meshgrid(np.linspace(0, 2 * np.pi, mask.shape[1]), np.arange(0, max_radius))
    x, y = coord_polar_to_cart(r, theta, center)
    x, y = np.round(x), np.round(y)
    x, y = x.astype(int), y.astype(int)
...
def cell_magic_wand_single_point(image, center, min_radius, max_radius, roughness=2, zoom_factor=1):
    ...
    polar_image = image_cart_to_polar(image,
                                      center,
                                      min_radius,
                                      max_radius,
    ...
    cart_mask = mask_polar_to_cart(polar_mask, center, min_radius, max_radius, image.shape, zoom_factor=zoom_factor)
    return cart_mask, cart_edge


def cell_magic_wand(image, center, min_radius, max_radius, roughness=2, zoom_factor=1, center_range=2):
    ...
    for i, c in enumerate(centers):
        mask, edge = cell_magic_wand_single_point(image,
                                                  c,
                                                  min_radius,
                                                  max_radius,
                                                  roughness=roughness,
...
def cell_magic_wand_3d(image_3d, center, min_radius, max_radius, roughness=2, zoom_factor=1, center_range=2, z_step=1):
    ...
    for s in range(int(image_3d.shape[0] / z_step)):
        mask = cell_magic_wand(image_3d[s * z_step, :, :],
                               center,
                               min_radius,
                               max_radius,
                               roughness=roughness,
