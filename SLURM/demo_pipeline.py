...
if preprocess:
    ...
    Yr, shifts, xcorrs, template = Yr.motion_correct(
        max_shift_w=10, max_shift_h=10,  method='opencv')
    max_h, max_w = np.max(shifts, axis=0)
    min_h, min_w = np.min(shifts, axis=0)
    Yr = Yr.crop(crop_top=max_h, crop_bottom=-min_h + 1,
...
cse.utilities.stop_server()
#%% save results
np.savez('results_analysis.npz', A=A.todense(), b=b, Cin=Cin, f_in=f_in, Ain=Ain, sn=sn, C=C, bl=bl, f=f, c1=c1, S=S, neurons_sn=neurons_sn,
         g=g, A2=A2.todense(), C2=C2, b2=b2, S2=S2, f2=f2, bl2=bl2, c12=c12, neurons_sn2=neurons_sn2, g21=g21, YrA=YrA, d1=d1, d2=d2)
