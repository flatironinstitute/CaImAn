        ...
        */
        stage('win-python3') {
          ...
          steps {
            ...
            bat 'set SRCPATH=%CD%'
            bat '%CONDA_ENV%\\scripts\\activate %CONDA_ENV% && pip install . && copy caimanmanager.py %TEMP% && cd %TEMP% && rmdir caiman_data /s /q && python caimanmanager.py install && python caimanmanager.py test'
          }
        }
      }
    }
