	...
	// With the CONDA_ENV variable on windows, you must be careful not to hit the maximum path length. 
        stage('win-python3') {
          ...
          steps {
            ...
            bat '%ANACONDA3%\\scripts\\activate %CONDA_ENV% && conda install numpy && pip install . && copy caimanmanager.py %TEMP% && cd %TEMP% && set "CAIMAN_DATA=%TEMP%\\caiman_data" && (if exist caiman_data (rmdir caiman_data /s /q && echo "Removed old caiman_data" ) else (echo "Host is fresh")) && python caimanmanager.py install --force && python caimanmanager.py test'
          }
        }
      }
    }
