...
def main_pipeline(params_movie, params_display):
    """
    Main pipeline for calcium analysis. Returns the data that was saved as a dictionary.
    :param params_movie: General parameters of the pipeline
    :param params_display: Specific display parameters
    :return: dict: Analyzed data
    """
    ...
    np.savez(os.path.join(os.path.split(fname_new)[0], os.path.split(fname_new)[1][:-4] + 'results_analysis.npz'),
             **to_save)
    # we save it
    # %%
...
if __name__=='__main__':
    ...
    params_movie = {'fname': ['Sue_2x_3000_40_-46.tif'],  # Don't edit - can be defined with GUI below
                    'niter_rig': 1,
    ...
    prefix = '.'  # Initial dir to open the file GUI - use '.' if you're not sure
    file_finder = FileFinder(prefix)
    # Comment the following line to use the default movies supplied with CaImAn
    params_movie['fname'] = file_finder.find_files()
    analyzed_data = main_pipeline(params_movie=params_movie, params_display=params_display)
